...
import { Mode, Direction } from '@/types';
...
const ShipmentList = () => {
  ...
  const ModeIcon = ({ mode }: { mode: Mode }) => {
    switch (mode) {
      ...
      case 'AIR': return <Plane className="w-4 h-4 text-purple-600" />;
      case 'ROAD': return <Truck className="w-4 h-4 text-orange-600" />;
    }
  };
        ...
        {selectedIds.size > 0 && (
          <div className="bg-blue-50 border-b border-blue-100 p-3 px-6 flex items-center justify-between transition-all animate-in slide-in-from-top-2">
            <div className="flex items-center gap-4">
              ...
              <button onClick={() => handleBulkAction('Update Status')} className="text-sm text-blue-700 hover:text-blue-900 font-medium flex items-center gap-2 px-2 py-1 hover:bg-blue-100 rounded">
                <RefreshCw className="w-4 h-4" /> Update Status
              </button>
              <button onClick={() => handleBulkAction('Generate Report')} className="text-sm text-blue-700 hover:text-blue-900 font-medium flex items-center gap-2 px-2 py-1 hover:bg-blue-100 rounded">
                <FileBarChart className="w-4 h-4" /> Generate Report
              </button>
            </div>
            <button onClick={() => setSelectedIds(new Set())} className="text-xs text-blue-500 hover:text-blue-700 font-medium">Clear Selection</button>
          </div>
        )}
          ...
          <table className="w-full text-left text-sm">
            ...
            <tbody className="divide-y divide-gray-100">
              {filtered.map(shipment => (
                <tr key={shipment.id} className={`hover:bg-gray-50 transition-colors group ${selectedIds.has(shipment.id) ? 'bg-blue-50/30' : ''}`}>
                  ...
                  <td className="px-6 py-4">
                     <div className="flex flex-col">
                        <Link href={`/shipments/${shipment.id}`} className="font-bold text-gray-900 group-hover:text-blue-600">
                            {shipment.reference}
                        </Link>
                  ...
                  <td className="px-6 py-4">
                     <div className="flex items-center gap-2 text-gray-700">
                        ...
                        <span className="text-sm">{shipment.mode.charAt(0) + shipment.mode.slice(1).toLowerCase()}</span>
                     </div>
                  </td>
                  <td className="px-6 py-4">
                  ...
                  <td className="px-6 py-4 text-gray-700">
                     <div className="w-24 leading-tight">
                        {getCommodityLabel(shipment.commodity)}
                     </div>
                  </td>
                  <td className="px-6 py-4">
