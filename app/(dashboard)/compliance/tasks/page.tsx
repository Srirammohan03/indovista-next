...
import { MOCK_COMPLIANCE_TASKS } from '@/data/mock';
import { Search, ArrowLeft, CheckCircle, XCircle, AlertCircle, Calendar, User } from 'lucide-react';
import Link from "next/link";
import { ComplianceTask, ComplianceTaskType } from '@/types';

const ComplianceTasks = () => {
  const [tasks, setTasks] = useState<ComplianceTask[]>(MOCK_COMPLIANCE_TASKS);
  
  const handleAction = (id: string, action: 'APPROVE' | 'REJECT') => {
      if(confirm(`Are you sure you want to ${action} this task?`)) {
          setTasks(tasks.map(t => t.id === id ? { ...t, status: action === 'APPROVE' ? 'APPROVED' : 'REJECTED' } : t));
      }
  };
  ...
  const getTypeLabel = (type: ComplianceTaskType) => {
      switch(type) {
          case 'KYC_REVIEW': return 'KYC Review';
          case 'SANCTIONS_CHECK': return 'Sanctions Check';
          case 'DOC_VALIDATION': return 'Doc Validation';
          default: return type;
      }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-6">
          ...
          {tasks.map(task => (
             <Card key={task.id} className={`border-l-4 ${task.status === 'PENDING' ? 'border-l-blue-500' : task.status === 'APPROVED' ? 'border-l-green-500' : 'border-l-red-500'} transition-all`}>
                 <div className="flex flex-col md:flex-row justify-between items-start gap-6">
                     <div className="flex-1">
                         ...
                         <h3 className="text-lg font-bold text-gray-900 mb-1">
                             {task.entityName}
                             <span className="text-sm font-normal text-gray-500 ml-2">({task.entityId})</span>
                         </h3>
